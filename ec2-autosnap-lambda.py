...
def lambda_handler(event, context):
    ...
    print("EC2 EBS snapshot backups stated at %s...\n" % datetime.now())
    for region in regions:
        print("Region: %s" % region)
        create_region_snapshots(region, retention_days)
    print("\nEc2 EBS snapshot backups completed at %s\n" % datetime.now())
...
# create and delete snapshots for volume
def snapshot_volumes(instance_name, retention_days, volumes):
    for v in volumes:
        ...
        delete_volume_snapshots(retention_days, v)
...
def delete_volume_snapshots(retention_days, volume):
