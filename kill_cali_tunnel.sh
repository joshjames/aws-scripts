...
aws ec2 stop-instances --region us-west-1 --instance-ids=$instance_id
echo "stopping $name instance id: $instance_id"
while state=$(aws ec2 describe-instances --region us-west-1 --instance-ids $instance_id --output text --query 'Reservations[*].Instances[*].State.Name'); test "$state" = "stopping"; do
  sleep 2; echo -n '.'
done; echo " $state"
echo $name stopped
